...
-->
        ...
        <div class="col-12 col-md-10 col-lg-8">
            <form class="card card-sm" action="<?= base_url('prov/index')?>" method="POST">
                <div class="card-body row no-gutters align-items-center">
                    ...
                    <div class="col">
                        <input id="cariProvinsi" class="form-control form-control-lg form-control-borderless" type="search" placeholder="Cari Nama Provinsi" name="keyword">
                    </div>
    ...
    <script type="text/javascript">
        var cari    = document.getElementById('cariProvinsi')
        var wrapper = document.getElementById('cariwrapper')

        cari.addEventListener('keyup', (e) => {
            // fetch('<?= base_url('prov/cari')?>', {method: 'POST',body: new URLSearchParams({'query': e.target.value})})
            ...
            $.ajax({
                type : 'POST',
                url : '<?= base_url('prov/cari')?>',
                data:{
                    'query': e.target.value 
                },
                success:function(data) {
                    var html = `<div class="row">${data}</div>` 
